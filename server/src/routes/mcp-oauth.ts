...
async function getClient(db: D1Database, clientId: string | undefined | null): Promise<ClientRow | null> {
...
async function issueTokens(db: D1Database, grant: { id: string; scope: KeyScope; clientName: string }) {
  const now = Date.now();
