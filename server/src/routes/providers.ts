...
// Env vars still win everywhere (see providers/config.ts's providerEnv) - these routes only
// manage the UI-configured fallback and never return a secret once saved.
...
providersRoutes.openapi(
  ...
  async (c) => {
    ...
    if (envConfigured(c.env, kind)) return c.json({ error: `${kind} is configured via environment variables (read-only)` }, 409);
