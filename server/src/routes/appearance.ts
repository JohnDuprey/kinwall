...
appearanceRoutes.openapi(
  ...
  async (c) => {
    const { themeMode, darkFrom, darkTo, accent, backgroundLight, backgroundDark, textScale, density } = await readSettings(c.env.DB);
    return c.json({ themeMode, darkFrom, darkTo, accent, backgroundLight, backgroundDark, textScale, density }, 200);
  },
);
