...
// (POST /api/calendars with the same account kind + remoteId, or PATCH { url } for ICS).
export const RECONNECT_MESSAGE = 'Reconnect this calendar to resume syncing';
...
export async function syncCalendar(env: Env, calendarId: string, execCtx?: WaitCtx): Promise<{ ok: true; count: number } | { ok: false; error: string }> {
  ...
  if (cal.config === '') return { ok: false, error: RECONNECT_MESSAGE };
...
export async function syncDue(env: Env, execCtx?: WaitCtx): Promise<void> {
  ...
  const { results } = await env.DB.prepare(
    `SELECT id FROM calendars WHERE kind != 'local' AND config != '' AND enabled = 1 AND (last_synced_at IS NULL OR last_synced_at < ?) ORDER BY last_synced_at IS NOT NULL, last_synced_at ASC`,
  )
    .bind(cutoff)
    .all<{ id: string }>();
