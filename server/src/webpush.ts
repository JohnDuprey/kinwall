...
export async function ensureVapidKeys(env: VapidEnv, db: KinwallDb): Promise<{ publicKey: string; privateKey: CryptoKey }> {
...
export async function getVapidPublicKey(env: VapidEnv, db: KinwallDb): Promise<string> {
...
// them plus the endpoint, a DB dump could push to the family's phones.
export type StoredPushSubscription = PushSubscriptionKeys & { id: string };

// Legacy rows written before encryption hold plaintext keys (no 'v1:' prefix; decrypt() passes
// them through). They're re-wrapped here after the next successful send - every send path goes
// through this function, so no startup pass or migration is needed.
async function rewrapLegacyKeys(env: VapidEnv, db: KinwallDb, sub: StoredPushSubscription, keys: PushSubscriptionKeys): Promise<void> {
  if (sub.p256dh.startsWith('v1:') && sub.auth.startsWith('v1:')) return;
  try {
    const [p256dh, auth] = await Promise.all([encrypt(env, keys.p256dh, sub.id), encrypt(env, keys.auth, sub.id)]);
    await db.prepare('UPDATE push_subscriptions SET p256dh = ?, auth = ? WHERE id = ?').bind(p256dh, auth, sub.id).run();
  } catch (err) {
    console.error(`push subscription ${sub.id} key re-wrap failed`, err);
  }
}
