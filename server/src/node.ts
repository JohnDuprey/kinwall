...
if (!(await isClaimed(db as unknown as D1Database))) {
  await regenerateSetupCode(db as unknown as D1Database, process.env.PUBLIC_URL || `http://localhost:${PORT}`);
}

if (haTimezone) {
  const existing = await (db as unknown as D1Database).prepare("SELECT value FROM settings WHERE key = 'timezone'").first();
  if (!existing) {
