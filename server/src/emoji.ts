// Shared single-emoji-grapheme validator (member avatars + chore emoji). Workers-compatible:
// Intl.Segmenter + Unicode property escapes only, no node:* APIs. Mirrored in web/src/emoji.ts —
// keep both in sync if the rule changes.
// ⃣ (combining enclosing keycap) covers keycaps like 1️⃣ - the base digit/#/* isn't
// itself Extended_Pictographic, but the keycap mark makes the whole grapheme an emoji.
