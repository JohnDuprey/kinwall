...
**Key scopes**: `admin` = everything. `display` = GET on members/calendars (config-free)/events/chores/settings/rev, event create/update/delete, chore create/update/complete/uncomplete, member & settings PATCH. `display` can NOT touch accounts, oauth, keys, webhooks, or calendar create/delete. Wall iPad uses a display key; Settings admin sections prompt for an admin key kept in sessionStorage for 5 min. `ADMIN_API_KEY` env is admin.
